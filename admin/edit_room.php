...
{
	$id=$_GET['edit'];
	$sql=$conn->query("SELECT * FROM room_category WHERE id='$id'");
	$abc=$sql->num_rows;
...
if (isset($_POST['update'])) 
{
 ...
 // $sql3=$conn->query("SELECT * FROM rooms WHERE room_cat='$room_type'");
  $num_room_same=$sql3->num_rows;
  ...
  if (!isset($errMSG)) 
  {
    ...
    $query="UPDATE room_category SET  roomname='$room_type', room_qnty='$num_room', available='$available', booked='$booked', no_bed='$num_bed', bedtype='$bed_type',   facility='$room_fec', price='$price', photo='$userpic' WHERE id='$id'";
     $result=$conn->query($query);

     $sql5=$conn->query("DELETE FROM rooms WHERE room_cat='$room'");
  
      for($i=0; $i<$num_room; $i++)
     {
       $query2=$conn->query("INSERT INTO rooms (room_cat, book) VALUES ('$room_type', 'false')");

      }
    if ($result===true && $query2===true) {
      ?>
      ...
      <div class="col-md-12">
        <form method="post" enctype="multipart/form-data">
          <div class="form-group">
            ...
            <input type="text" name="r_type" class="form-control" value="<?php echo $result['roomname']; ?>">
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <textarea name="fec_room" class="form-control" rows="3" ><?php echo $result['facility'];?></textarea>
          </div>
          <div class="form-group">
            <label>Enter Your Room Image</label><br>
            <p><img src="user_images/<?php echo $result['photo']; ?>" height="150" width="150"></p>
          ...
          <div class="form-group">
            <input type="submit" name="update" value="Update Room" class="form-control btn btn-primary">
          </div>
        </form>
      </div>
    </div>
